...
namespace rpos { namespace system { namespace util {
    ...
    public:
        boost::function<void(const boost::system::error_code&, const T&)> getCallback()
        {
            return boost::bind(&AsyncSynchronizer::onCallback, this->shared_from_this(),
    ...
    private:
        static void onCallback(boost::weak_ptr<AsyncSynchronizer> weakSelf, const boost::system::error_code& ec, const T& result)
        {
    ...
    namespace detail {
        ...
        struct await_types_
        {
            typedef boost::function<void(const boost::system::error_code&, const T&)> callback;
